...
// ЛОГИРОВАНИЕ ДЛЯ ОТЛАДКИ: логируем ВСЕ обновления
bot.use((ctx, next) => {
  if (ctx.message) {
    console.log(`📨 [MIDDLEWARE] Обновление: тип=${ctx.updateType}, текст="${(ctx.message as any).text || 'нет текста'}"`);
  }
  return next();
});
...
// ═══════════════════════════════════════════════════════════
// ОСНОВНЫЕ КОМАНДЫ (со слешем / префиксом)
// ═══════════════════════════════════════════════════════════
...
export async function startBot() {
  ...
  bot.launch().catch((err: any) => console.error('❌ Ошибка запуска:', err?.message));
